...
<html>
  ...
  <body>
    <form action="/mynews/servlet/userServlet?type=deleteUser&page=1" method="post" id="myform">
		<table border="frame" rules="all" class="usr_table">
			...
			<c:forEach items="${requestScope.list}" var="user">
				<tr>
					...
					<td>${user.enable} </td>
					<td><input type="checkbox" name="checkbox1" value="${user.id}"></td>
				</tr>
			</c:forEach>
			<tr>
	...
	<div class="page_div">
		...
		<c:choose>
			<c:when test="${ page==i}">
			<a href="/mynews/servlet/userServlet?type=deleteUser&page=${i}">
		...
		<a href="/mynews/servlet/userServlet?type=deleteUser&page=${lastpage}">
